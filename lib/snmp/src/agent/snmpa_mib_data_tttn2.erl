...
-define(DEFAULT_TREE, {tree, {0, []}, internal}).
...
do_load_mib(MibData, ActualFileName, MibName, MeOverride, TeOverride) ->
    ...
    case (catch merge_nodes(T, OldRoot)) of
        {error_merge_nodes, _Node1, _Node2, Oid} ->
            ?vlog("error merging nodes:"
                "~n~p~nand~n~p", [_Node1, _Node2]),
            {error, {oid_conflict, Oid}};
        NewRoot when is_tuple(NewRoot) andalso (element(1,NewRoot) =:= tree) ->
            ...
            case (catch check_notif_and_mes(TeOverride, MeOverride, Symbolic,
				...
				MibDb, Symbolic, Mib,
                    ...
                    %% Mod:write(TreeDb, Tree2), %% Store later?
...
check_notifications([_Crap | Traps]) ->
    ?vlog("skipped check of: ~n~p",[_Crap]),
    check_notifications(Traps).
...
info(MibData) ->
    ...
    [{loaded_mibs,     LoadedMibs}, 
     {subagents,       SAs}, 
     {tree_size_bytes, TreeSize},
     {process_memory,  ProcSize},
     {db_memory,       [{mib,  MibDbSize}, 
			{node, NodeDbSize}, 
			{tree, TreeDbSize}]}].
...
find_node(D, {tree, Tree, _Internal}, [Int | RestOfOid], RevOid) ->
    ...
    find_node(D, sparse_element(Int, Tree), RestOfOid, [Int | RevOid]);
find_node(D, {node, {table_column, _}}, _RestOfOid, [ColInt | RevOid]) ->
     ?vtrace("find_node(tree,table_column) -> entry with"
         "~n   RestOfOid: ~p"
         "~n   ColInt:    ~p"
         "~n   RevOid:    ~p",[_RestOfOid, ColInt, RevOid]),
    #mib_data{module = Mod, node_db = Db} = D,
    ...
    case Mod:read(Db, Oid) of
        ...
        false ->
            _X = Mod:read(Db, lists:reverse([ColInt | RevOid])),
            ?vinfo("find_node -> could not find table_column ME with"
                "~n   RevOid: ~p"
                "~n   trying  [~p|~p]"
                "~n   X:      ~p",
                [RevOid, [ColInt | RevOid], _X]),
            false
    end;
find_node(D, {node, {variable, _MibName}}, [0], RevOid) ->
    ...
    #mib_data{module = Mod, node_db = Db} = D,
    Oid = lists:reverse(RevOid),
    %% {value, #node_info{me = ME}} = Mod:read(Db, Oid),
    case Mod:read(Db, Oid) of
        {value, #node_info{me = ME, mib_name = Mib}} ->
            {variable, ME, Mib};
        false ->
...
find_node(_D, _Node, _RestOfOid, _RevOid) ->
    ?vtrace("find_node -> failed:~n~p",[_Node]),
    {false, noSuchObject}.
...
next_node(_D, {tree, Tree, {table_entry, _Id}}, [Int | _Oid],
  ...
  when Int + 1 > element(1, Tree) ->
    ?vdebug("next_node(tree,table_entry) -> entry when not found"
	    "~n   Int:          ~p"
	    "~n   size of Tree: ~p", [Int, element(1, Tree)]),
    false;
next_node(D, {tree, _Tree, {table_entry, _MibName}},
    ...
    ?vtrace("next_node(tree,table_entry) -> entry when"
	    ...
	    "~n   RevOidSoFar:  ~p"
	    "~n   MibView:      ~p", 
	    [element(1, _Tree), Oid, RevOidSoFar, MibView]),
        ...
        _ ->
            ...
            case Mod:read(Db, OidSoFar) of
                false ->
                    ...
                    false;
                {value, #node_info{me = ME}} ->
                    ?vtrace("next_node(tree,table_entry) -> found node info: "
			    "~n   OidSoFar: ~p"
			    "~n   ME:       ~p", [OidSoFar, ME]),
...
next_node(D, {tree, Tree, _Info} = T, [Int | RestOfOid], RevOidSoFar, MibView)
  when (Int < element(1, Tree)) andalso (Int >= 0) ->
    ?vtrace("next_node(tree) -> entry when"
	    ...
	    "~n   RevOidSoFar:  ~p"
	    "~n   MibView:      ~p",
        [element(1, Tree), Int, RestOfOid, RevOidSoFar, MibView]),
    case next_node(D, sparse_element(Int, Tree),
                   RestOfOid, [Int | RevOidSoFar], MibView) of
        false ->
	    ?vtrace("next_node(tree) -> false - find next", []), 
            NextInt = sparse_next_defined(Int, Tree),
	    ?vtrace("next_node(tree) -> Next int: ~p", [NextInt]), 
            find_next(D, T, NextInt, RevOidSoFar, MibView);
        Else ->
...
next_node(D, {tree, Tree, _Info} = T, [], RevOidSoFar, MibView) ->
    ?vtrace("next_node(tree,[]) -> entry when"
	    ...
	    "~n   RevOidSoFar: ~p"
	    "~n   MibView:     ~p",
	    [element(1, Tree), RevOidSoFar, MibView]),
    find_next(D, T, 0, RevOidSoFar, MibView);
next_node(_D, {tree, _Tree, _Info}, _RestOfOid, _RevOidSoFar, _MibView) ->
    ?vtrace("next_node(tree) -> entry when not found"
	    "~n   size(Tree):  ~p", [element(1, _Tree)]),
    false;
...
find_next(D, {tree, Tree, internal}, Idx, RevOidSoFar, MibView)
  when Idx < element(1, Tree) ->
    ?vtrace("find_next(tree,internal) -> entry when"
	    "~n   size of Tree: ~p"
	    "~n   Idx:          ~p"
	    "~n   RevOidSoFar:  ~p"
	    "~n   MibView:      ~p", 
	    [element(1, Tree), Idx, RevOidSoFar, MibView]),
    case find_next(D, 
		   sparse_element(Idx, Tree), 
		   0, [Idx | RevOidSoFar], MibView) of
        false ->
	    ?vtrace("find_next(tree,internal) -> false - try next idx", []), 
            NextIdx = sparse_next_defined(Idx, Tree),
            find_next(D, {tree, Tree, internal}, NextIdx, RevOidSoFar, MibView);
        Other ->
            Other
    end;
...
convert_tree({Index, {tree, Tree, Info}}) when Index >= 0 ->
    ...
    {Index, {tree, dict_list_to_sparse_tuple_us(L), Info}};
convert_tree({Index, {node, Info}}) when Index >= 0 ->
    {Index, {node, Info}};
convert_tree(Tree) when is_list(Tree) ->
    L = lists:map(fun convert_tree/1, Tree),
    dict_list_to_sparse_tuple_us(L).
...
merge_nodes(Same, Same, _Oid) ->
    Same;
merge_nodes({tree, Tree1, internal}, {tree, Tree2, internal}, Oid) ->
    {tree, merge_levels(Tree1, Tree2, Oid), internal};
merge_nodes(Node1, Node2, Oid) ->
    throw({error_merge_nodes, Node1, Node2, lists:reverse(Oid)}).
...
%%----------------------------------------------------------------------
merge_levels(T1, T2, Oid) ->
    dict_list_to_sparse_tuple(
        merge_levels(
            sparse_tuple_to_dict_list(T1), sparse_tuple_to_dict_list(T2), [],
            Oid)).

merge_levels([{Idx1, _} = H1 | T1], L2 = [{Idx2, _} | _], Acc, Oid)
        when Idx1 < Idx2 ->
    merge_levels(T1, L2, [H1 | Acc], Oid);
merge_levels(L1 = [{Idx1, _} | _], [H2 = {Idx2, _} | T2], Acc, Oid)
        when Idx1 > Idx2 ->
    merge_levels(L1, T2, [H2 | Acc], Oid);
merge_levels(L1, [], Acc, _Oid) ->
    lists:reverse(Acc) ++ L1;
merge_levels([], L2, Acc, _Oid) ->
    lists:reverse(Acc) ++ L2;
merge_levels([H | T1], [H | T2], Acc, Oid) ->
    merge_levels(T1, T2, [H | Acc], Oid);
merge_levels([{Idx, Data1} | T1], [{Idx, Data2} | T2], Acc, Oid) ->
    merge_levels(T1, T2, [{Idx, merge_nodes(Data1, Data2, [Idx | Oid])} | Acc],
                 Oid).
...
delete_subagent({tree, Tree, Info}, [Index]) ->
    {node, subagent} = sparse_element(Index, Tree),
    {tree, set_sparse_element(Index, Tree, undefined_node), Info};
delete_subagent({tree, Tree, Info}, [Index | TI]) ->
    {tree, set_sparse_element(Index, Tree,
                      delete_subagent(sparse_element(Index, Tree), TI)), Info}.
...
%% install_mibs(Mod, NodeDb, MibNames) ->
    InstallME = 
	fun(#node_info{me = ME}) ->
		call_instrumentation(ME, new)
	end,
    InstallMib = 
	fun(MibName) ->
		Pattern = #node_info{mib_name = MibName, _ = '_'},
		lists:foreach(InstallME, Mod:match_object(NodeDb, Pattern))
	end,
    lists:foreach(InstallMib, MibNames).
...
install_mes(_Mod, _Db, _MibName, []) ->
    ok;
install_mes(Mod, Db, MibName, [ME|MEs]) ->
...
uninstall_mib(Mod, Db, Symbolic, MibName, MEs) ->
    ...
    _Res = Mod:delete(Db, MibName),
    ?vtrace("uninstall_mib -> (mib) db delete result: ~p", [_Res]),
...
%% Output: A tuple where element Index + 3 is Data, unless data is sparse,
%%         (considered sparse if containing sequences of more than 9
%%          undefined_node) in which case empty sequences are denoted in
%%         2nd element.
%%         1st element is the "sparse size", i.e. the number of elements
%%         that can be looked up in the tuple seen as a zero-based array.
%%         Index of last element is "sparse size" - 1.
%%
%% Example:
%%          Input [{1,one},{2,two},{4,four},{22,twentytwo},{42,fortytwo}]
%%          Output {43,
%%                  [{{5,22},17},        %% 5-21 are undefined_node
%%                   {{23,42},36}],      %% 23-41 are undefined_node
%%                  undefined_node,      %% Node 0 kept for historical
%%                                       %% reasons. Seems superfluous.
%%                  one,
%%                  two,
%%                  undefined_node,      %% Short sequences kept as-is.
%%                  four,
%%                  twentytwo,           %% Any sequence of more than 9
%%                  fortytwo}            %% undefined_node skipped.
%%
%%----------------------------------------------------------------------

dict_list_to_sparse_tuple_us(L) ->
    LS = lists:keysort(1, L),
    dict_list_to_sparse_tuple(LS).

dict_list_to_sparse_tuple(L) ->
    list_to_tuple(integrate_indexes(L)).

integrate_indexes(DictList) ->
    integrate_indexes(0, DictList, [], []).

integrate_indexes(CurIndex, [{CurIndex, Data} | T], Skips, SparseList) ->
    integrate_indexes(CurIndex + 1, T, Skips, [Data|SparseList]);
integrate_indexes(Index, [], Skips, SparseList) ->
    [Index, lists:reverse(Skips)|lists:reverse(SparseList)];
integrate_indexes(CurIndex, [{NextIndex, Data} | T], Skips, SparseList)
        when  NextIndex > CurIndex + 9 ->
    integrate_indexes(
        NextIndex + 1, T,
        [{{CurIndex, NextIndex}, NextIndex - CurIndex + prev_skip(Skips)} |
         Skips],
        [Data | SparseList]);
integrate_indexes(CurIndex, L, Skips, SparseList) ->
    integrate_indexes(CurIndex + 1, L, Skips, [undefined_node | SparseList]).

prev_skip([]) -> 0;
prev_skip([{_,LastSkip} | _]) -> LastSkip.


%%----------------------------------------------------------------------
%% Purpose: Various lookup/update/convert functions for working with
%%          sparse tuple trees.
%%
%%          See format of tree level above.
%%
%%----------------------------------------------------------------------

%% lookup
sparse_element(N, SparseTuple) when element(2, SparseTuple) =:= [] ->
    element(N + 3, SparseTuple);
sparse_element(N, SparseTuple) ->
    Skips = element(2, SparseTuple),
    case find_skip(N, Skips) of
        undefined_node ->
            undefined_node;
        Skip ->
            element(N + 3 - Skip, SparseTuple)
    end.

find_skip(N, Skips) ->
    find_skip(N, Skips, 0).

find_skip(N, [{{Start,_}, _} | _], Skip) when N < Start ->
    Skip;
find_skip(N, [{{Start, End},_} | _], _) when Start =< N, N < End ->
    undefined_node;
find_skip(_, [], P) -> P;
find_skip(N, [{_, P} | T], _) ->
    find_skip(N, T, P).
...
sparse_tuple_to_dict_list(SparseTuple) when is_tuple(SparseTuple) ->
    sparse_tuple_to_dict_list(tl(tuple_to_list(SparseTuple)));
sparse_tuple_to_dict_list([Skips | SparseList]) ->
    sparse_tuple_to_dict_list(Skips, SparseList, 0, []).

sparse_tuple_to_dict_list(_, [], _, DictList) ->
    lists:reverse(DictList);
sparse_tuple_to_dict_list([{{Idx, End}, _} | T], SparseList, Idx, DictList) ->
    sparse_tuple_to_dict_list(T, SparseList, End, DictList);
sparse_tuple_to_dict_list(Skip, [undefined_node|T], Idx, DictList)  ->
    sparse_tuple_to_dict_list(Skip, T, Idx + 1, DictList);
sparse_tuple_to_dict_list(Skip, [H|T], Idx, DictList) ->
    sparse_tuple_to_dict_list(Skip, T, Idx + 1, [{Idx, H} | DictList]).

%% update - not yet optimized
set_sparse_element(N, SparseTuple, Element) ->
    dict_list_to_sparse_tuple(
        set_dict_list_element(
            N,sparse_tuple_to_dict_list(SparseTuple),Element)).

set_dict_list_element(N, DictList, Element) ->
    set_dict_list_element(N, DictList, [], Element).

set_dict_list_element(_, [], Acc, undefined_node) ->
    lists:reverse(Acc);
set_dict_list_element(N, [], Acc, Element) ->
    lists:reverse([{N, Element} | Acc]);
set_dict_list_element(N, [{M, _} | _] = T, Acc, undefined_node) when M > N ->
    lists:reverse(Acc) ++ T;
set_dict_list_element(N, [{M, _} | _] = T, Acc, Element) when M > N ->
    lists:reverse([{N, Element} | Acc]) ++ T;
set_dict_list_element(N, [{N, _} | T], Acc, undefined_node) ->
    lists:reverse(Acc) ++ T;
set_dict_list_element(N, [{N, _} | T], Acc, Element) ->
    lists:reverse([{N, Element} | Acc]) ++ T;
set_dict_list_element(N, [H|T], Acc, Element) ->
    set_dict_list_element(N, T, [H | Acc], Element).

%% get index of next element /= undefined_node
sparse_next_defined(Idx, SparseTuple) ->
    Skips = element(2, SparseTuple),
    sparse_next_defined(Idx, SparseTuple, Skips).

sparse_next_defined(Idx, SparseTuple, _)
  when element(1, SparseTuple) =< (Idx + 1) ->
    %% end of tuple - handle in caller
    Idx + 1;
sparse_next_defined(Idx, SparseTuple, []) ->
    case sparse_element(Idx + 1, SparseTuple) of
        undefined_node ->
            sparse_next_defined(Idx + 1, SparseTuple, []);
        _ ->
            Idx + 1
    end;
sparse_next_defined(Idx, SparseTuple, [{{_From, To}, _} | TSkips])
  when To =< Idx ->
    sparse_next_defined(Idx, SparseTuple, TSkips);
sparse_next_defined(Idx, _SparseTuple, [{{From, To}, _} | _])
  when From =< Idx -> % Idx < To
    To;
sparse_next_defined(Idx, SparseTuple, Skips) ->
    case sparse_element(Idx + 1, SparseTuple) of
        undefined_node ->
            sparse_next_defined(Idx + 1, SparseTuple, Skips);
        _ ->
            Idx + 1
    end.
